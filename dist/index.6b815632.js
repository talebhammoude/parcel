...
function readAllData() {
    ...
    const dbRef = _database.ref(db, 'messages/'); //En referens till hela databasen
    _database.onValue(dbRef, (snapshot)=>{
        ...
        dataToArr.forEach((element)=>{
            const theList1 = document.querySelector(".chat-box");
            const li = document.createElement("li");
            li.appendChild(document.createTextNode(element.username + ": "));
            li.appendChild(document.createTextNode(element.theMsg));
            theList1.appendChild(li);
        });
    });
}
readAllData();
const inputField = document.querySelector(".msg-input-field");
const sendButton = document.querySelector(".send-btn");
const theList = document.querySelector(".chat-box");
function writeMsgData(userId, name, theMsg) {
    const db = _database.getDatabase();
    _database.set(_database.ref(db, 'messages/' + userId), {
        username: name,
        theMsg: theMsg
    });
}
...
},{}]},["g75ug","kuM8f"], "kuM8f", "parcelRequire2f31")
